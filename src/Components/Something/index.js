  ...
  render() {
    ...
    const content = {
      ...
      ProductionManagers: {
        ...
        detials: [
          "Create production plans & timelines for each task.",
          "Stay on-top of production process with live updates.",
          "Monitor & improve the efficiency of machines & operators.",
          "Anticipate machine maintenance using historic reports & deep analytics.",
          "Create role-based training modules for your employees & track their progress",
        ],
      },
      Supervisors: {
        title: "Supervisors",
        detials: [
          "Save time by remotely monitoring the progress.",
          "Resolve issues promptly by quickly escalating alerts to the concerned departments.",
          "Ensure smooth production by identifying & fixing issues.",
        ],
      },
      Operators: {
        title: "Operators",
        detials: [
          "Stop worrying about filling manual reports.",
          "Always stay on the target by keeping track of the progress.",
          "Rise through the ranks using detailed performance reports and internal training modules.",
        ],
      },
    };
    return (
            ...
            <Box mt="60px">
              {details.map((eachEle, idx) => (
                <Box className={classes.somethingList} key={idx}>
                  <Box
                    className={
                      selected === eachEle.title.split(" ").join("")
                        ? classes.selected
                        : classes.notSelected
                    }
                  ></Box>
          ...
          <Box>
            {content[selected]["detials"].map((eachEle, idx) => (
              <Box display="flex" alignItems="baseline" key={idx}>
                ...
                <Box className={classes.contentDetails} flex="1">
                  {eachEle}
                </Box>
              </Box>
            ))}
